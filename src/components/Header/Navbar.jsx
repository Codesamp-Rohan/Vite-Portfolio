  ...
  return (
    <>
      <div
        className="menu--area"
        style={{
          transform: isOpen ? "translate(0, 0)" : "translate(200%, -200%)",
        }}>
        <button
          className="absolute w-[20px] right-[6%] top-[4%]"
          onClick={menuOption}>
          <img className="invert w-[100%]" src="/Icons/close.png"></img>
        </button>
        <ul className="w-full h-full flex flex-col justify-center pl-[30px]">
          {navLinks.map((links, index) => {
            return (
              <li className="text-[3rem] nav--links" key={index}>
        ...
        {navLinks.map((links, index) => {
          return (
            <li
              ...
              key={index}>
              <Link onClick={handleActive} to={links.link}>
                {links.name}
              </Link>
            </li>
          );
